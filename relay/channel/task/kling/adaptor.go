...
func (a *TaskAdaptor) BuildRequestURL(info *relaycommon.RelayInfo) (string, error) {
	path := getKlingVideoPath(info.Action)
...
func (a *TaskAdaptor) BuildRequestBody(c *gin.Context, info *relaycommon.RelayInfo) (io.Reader, error) {
	...
	body := a.buildRequestMap(&req)

	// Determine action based on image presence
	image, _ := body["image"].(string)
	imageTail, _ := body["image_tail"].(string)
	if image == "" && imageTail == "" {
		// Don't override action for omni-video requests
		if c.GetString("action") != constant.TaskActionOmniGenerate {
...
func (a *TaskAdaptor) GetModelList() []string {
	return []string{"kling-v1", "kling-v1-6", "kling-v2-master", "kling-v3-omni", "kling-video-o1"}
}
...
func (a *TaskAdaptor) ParseTaskResult(respBody []byte) (*relaycommon.TaskInfo, error) {
	...
	// 解析可灵点消耗，存入 TotalTokens 用于后续重新计费
	// 注意：可灵 API 可能返回小数值如 "2.5"，必须用 ParseFloat 解析
	if resPayload.Data.FinalUnitDeduction != "" {
		if points, err := strconv.ParseFloat(resPayload.Data.FinalUnitDeduction, 64); err == nil && points > 0 {
			taskInfo.TotalTokens = int(math.Ceil(points))
		}
	}
	return taskInfo, nil
}
