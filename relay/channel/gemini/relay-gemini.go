...
func CovertOpenAI2Gemini(c *gin.Context, textRequest dto.GeneralOpenAIRequest, info *relaycommon.RelayInfo) (*dto.GeminiChatRequest, error) {
	...
	if len(textRequest.ExtraBody) > 0 {
		...
		if googleBody, ok := extraBody["google"].(map[string]interface{}); ok {
			...
			// Auto-convert camelCase imageConfig to snake_case image_config
			if camelImageConfig, hasCamel := googleBody["imageConfig"]; hasCamel {
				if _, hasSnake := googleBody["image_config"]; !hasSnake {
					googleBody["image_config"] = camelImageConfig
				}
			}

			if imageConfig, ok := googleBody["image_config"].(map[string]interface{}); ok {
				// Auto-convert camelCase aspectRatio to snake_case aspect_ratio
				if ar, hasCamel := imageConfig["aspectRatio"]; hasCamel {
					if _, hasSnake := imageConfig["aspect_ratio"]; !hasSnake {
						imageConfig["aspect_ratio"] = ar
					}
				}
				// Auto-convert camelCase imageSize to snake_case image_size
				if is, hasCamel := imageConfig["imageSize"]; hasCamel {
					if _, hasSnake := imageConfig["image_size"]; !hasSnake {
						imageConfig["image_size"] = is
					}
				}
...
func geminiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response, callback func(data string, geminiResponse *dto.GeminiChatResponse) bool) (*dto.Usage, int, *types.NewAPIError) {
	var usage = &dto.Usage{}
	var imageCount int
		...
		for _, candidate := range geminiResponse.Candidates {
			for _, part := range candidate.Content.Parts {
				if part.InlineData != nil && part.InlineData.MimeType != "" {
					imageCount++
				} else if part.Text != "" && hasImageURLInText(part.Text) {
					imageCount++
				}
				if part.Text != "" {
...
func GeminiChatStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	usage, imageCount, err := geminiStreamHandler(c, info, resp, func(data string, geminiResponse *dto.GeminiChatResponse) bool {
	...
	// CUSTOM: 图片生成模型流式检查：如果没有生成任何图片，记录警告并返回错误
	if model_setting.IsGeminiModelSupportImagine(info.UpstreamModelName) && imageCount == 0 {
		logger.LogWarn(c, "image generation model returned no images in streaming mode")
		common.SetContextKey(c, constant.ContextKeyAdminRejectReason, "gemini_image_gen_no_image_stream")
		return usage, types.NewOpenAIError(
			errors.New("image generation failed: no image returned by Gemini API"),
...
func GeminiChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	// CUSTOM(Feature 1): 图片生成模型检查 - finishReason!=STOP 或没图片返回错误不扣费
	if model_setting.IsGeminiModelSupportImagine(info.UpstreamModelName) {
		for _, candidate := range geminiResponse.Candidates {
			if candidate.FinishReason != nil && *candidate.FinishReason != "STOP" {
				finishReason := *candidate.FinishReason
				common.SetContextKey(c, constant.ContextKeyAdminRejectReason, fmt.Sprintf("gemini_image_gen_failed_reason=%s", finishReason))
				failUsage := buildUsageFromGeminiMetadata(geminiResponse.UsageMetadata, info.GetEstimatePromptTokens())
				return &failUsage, types.NewOpenAIError(errors.New(fmt.Sprintf("image generation failed: finish_reason=%s", finishReason)), types.ErrorCodeEmptyResponse, http.StatusInternalServerError, types.ErrOptionWithSkipRetry())
			}
		}
		if !hasImageInGeminiResponse(geminiResponse.Candidates) {
			common.SetContextKey(c, constant.ContextKeyAdminRejectReason, "gemini_image_gen_no_image")
			failUsage := buildUsageFromGeminiMetadata(geminiResponse.UsageMetadata, info.GetEstimatePromptTokens())
			return &failUsage, types.NewOpenAIError(errors.New("image generation failed: no image in response (model may have refused to generate)"), types.ErrorCodeEmptyResponse, http.StatusInternalServerError, types.ErrOptionWithSkipRetry())
		}
	}
