...
func tencentStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	for scanner.Scan() {
		...
		if len(response.Choices) != 0 {
			responseTokens.WriteString(response.Choices[0].Delta.GetContentString())
		}
	...
	return service.ResponseTokens2Usage(c, responseTokens.Tokens(), info.GetEstimatePromptTokens()), nil
}
