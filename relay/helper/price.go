...
func ModelPriceHelper(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta) (types.PriceData, error) {
	...
	if !usePrice {
		...
		// CUSTOM(Feature 3): Gemini Image Size 倍率应用到 modelRatio (token-based pricing path)
		// 模型名带 -token 后缀时跳过图片尺寸倍率（按纯 token 计费）
		if meta.ImagePriceRatio > 0 && !strings.HasSuffix(info.OriginModelName, "-token") {
			modelRatio = modelRatio * meta.ImagePriceRatio
		}
