...
func (r *GeneralOpenAIRequest) GetTokenCountMeta() *types.TokenCountMeta {
	...
	// CUSTOM(Feature 3): Extract image_size from extra_body.google.image_config for Gemini image pricing
	if len(r.ExtraBody) > 0 {
		...
		if err := common.Unmarshal(r.ExtraBody, &extraBody); err == nil {
			if googleBody, ok := extraBody["google"].(map[string]interface{}); ok {
				...
				if imageConfig != nil {
					imageSize, _ := imageConfig["image_size"].(string)
					if imageSize == "" {
						imageSize, _ = imageConfig["imageSize"].(string)
					}
					if imageSize != "" {
						if sizeRatio, found := ratio_setting.GetGeminiImageSizeRatio(imageSize); found {
							tokenCountMeta.ImagePriceRatio = sizeRatio
						}
					}
				}
			}
