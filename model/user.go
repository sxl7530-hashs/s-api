...
type User struct {
	...
	RebatePercent    float64                    `json:"rebate_percent" gorm:"default:-1;column:rebate_percent"` // 个人返利比例，-1 表示使用全局设置
...
// InvitedUserInfo contains info about an invited user and their top-up statistics
type InvitedUserInfo struct {
	...
	TotalTopUp  int64  `json:"total_topup" gorm:"column:total_topup"`
	TopUpCount  int    `json:"topup_count" gorm:"column:topup_count"`
	UsedQuota   int    `json:"used_quota" gorm:"column:used_quota"`
}

// GetInvitedUsers returns users invited by the given inviter, with their top-up stats
func GetInvitedUsers(inviterId int, pageInfo *common.PageInfo) (users []InvitedUserInfo, total int64, err error) {
	...
	query := fmt.Sprintf(`
		SELECT u.id, u.username, u.display_name, u.status, u.created_at, u.used_quota,
		...
		ORDER BY u.id DESC
		LIMIT ? OFFSET ?
	`, userTable, topupTable)

	err = DB.Raw(query, common.TopUpStatusSuccess, inviterId, pageInfo.GetPageSize(), pageInfo.GetStartIdx()).Scan(&users).Error
	if err != nil {
		return nil, 0, err
	}
