...
export const useChannelsData = () => {
  ...
  const manageChannel = async (id, action, record, value) => {
    ...
    if (success) {
      ...
      if (action === 'enable') {
        ...
        // 其他操作（priority/weight/enable_all）后端返回完整 channel 对象
        let channel = res.data.data;
        if (channel && typeof channel === 'object' && 'status' in channel) {
  ...
  const testSelectedChannelModels = async () => {
    if (selectedChannels.length === 0) {
    ...
    try {
      const res = await API.post('/api/channel/test/models/batch', {
        channel_ids: channelIds,
      });
      const { success, message } = res.data;
      if (success) {
        showSuccess(
          t('已开始测试所选 {{count}} 个渠道的全部模型', {
            count: channelIds.length,
          }),
        );
        setSelectedChannels([]);
      } else {
        showError(message || t('启动批量模型测试失败'));
      }
    } catch (error) {
      showError(error?.message || t('启动批量模型测试失败'));
    }
  };
