...
/**
 * Render created_at as relative time with full date tooltip
 */
const renderCreatedAt = (timestamp, t) => {
  ...
  if (seconds < 60) {
    ...
    relativeTime = `${minutes}${t('分钟前')}`;
  } else if (hours < 24) {
    relativeTime = `${hours}${t('小时前')}`;
  } else if (days < 30) {
    relativeTime = `${days}${t('天前')}`;
  } else if (days < 365) {
    relativeTime = `${Math.floor(days / 30)}${t('个月前')}`;
  } else {
    relativeTime = `${Math.floor(days / 365)}${t('年前')}`;
  }

  const fullDate = date.toLocaleString();
  return (
    <Tooltip content={relativeTime} position='top'>
      <span style={{ color: 'var(--semi-color-text-2)', fontSize: '13px', cursor: 'default' }}>
        {fullDate}
      </span>
    </Tooltip>
  );
...
export const getUsersColumns = ({
  ...
  return [
    ...
    },
    {
      ...
      render: (text) => renderCreatedAt(text, t),
      width: 120,
    },
    {
      title: t('最后登录'),
