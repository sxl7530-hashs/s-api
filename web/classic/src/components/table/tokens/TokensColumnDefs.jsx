...
import { formatQuotaCompact } from '../../../helpers/quota';
import {
...
const renderGroupColumn = (text, record, t, groupRatios = {}) => {
  const profile = record?.token_group_profile;
  if (text === 'auto') {
    return (
      <div className='flex flex-col gap-1'>
        {profile && (
          <Tooltip content={profile.description || profile.name} position='top'>
  ...
  return (
    <Popover content={popoverContent} position='top'>
      <Tag
        color='white'
        shape='circle'
        style={{ maxWidth: 190, overflow: 'hidden' }}
      >
        <div
          className='flex min-w-0 flex-col overflow-hidden'
          aria-label={`${t('剩余额度')}: ${renderQuota(remain)}; ${t('总额度')}: ${renderQuota(total)}; ${t('已用额度')}: ${renderQuota(used)}`}
        >
          <div className='flex min-w-0 items-center justify-between gap-2 text-xs leading-none'>
            <span className='min-w-0 truncate tabular-nums'>
              {formatQuotaCompact(remain)}
            </span>
            <span className='min-w-0 truncate text-right tabular-nums'>
              {formatQuotaCompact(total)}
            </span>
          </div>
          <Progress
            percent={percent}
            ...
            style={{ width: '160px', marginTop: '1px', marginBottom: 0 }}
          />
        </div>
      </Tag>
    </Popover>
