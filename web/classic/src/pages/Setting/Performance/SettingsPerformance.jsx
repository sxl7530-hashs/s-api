...
export default function SettingsPerformance(props) {
  ...
  const [inputs, setInputs] = useState({
    ...
    'performance_setting.disk_cache_max_size_mb': 0,
    'performance_setting.disk_cache_path': '',
    'performance_setting.disk_cache_critical_watermark_percent': 90,
    'performance_setting.disk_cache_unknown_length_disk_first': true,
    'performance_setting.disk_cache_max_request_mb': 4096,
    'performance_setting.disk_cache_auto_sizing': true,
    'performance_setting.disk_cache_max_disk_percent': 50,
    'performance_setting.disk_cache_min_free_space_mb': 30720,
        ...
        >
          <Form.Section text={t('磁盘缓存设置（磁盘换内存）')}>
            ...
            <Row gutter={16}>
              ...
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.Switch
                  field={'performance_setting.disk_cache_auto_sizing'}
                  label={t('根据磁盘容量自动设置缓存上限')}
                  extraText={t('按磁盘容量和预留空间动态计算缓存上限')}
                  onChange={handleFieldChange(
                    'performance_setting.disk_cache_auto_sizing',
                  )}
                  disabled={!inputs['performance_setting.disk_cache_enabled']}
                />
              </Col>
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.InputNumber
                  field={'performance_setting.disk_cache_max_disk_percent'}
                  label={t('缓存最多使用磁盘 (%)')}
                  extraText={t('自动模式使用的最大磁盘比例')}
                  min={1}
                  max={90}
                  onChange={handleFieldChange(
                    'performance_setting.disk_cache_max_disk_percent',
                  )}
                  disabled={
                    !inputs['performance_setting.disk_cache_enabled'] ||
                    !inputs['performance_setting.disk_cache_auto_sizing']
                  }
                />
              </Col>
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.InputNumber
                  field={'performance_setting.disk_cache_min_free_space_mb'}
                  label={t('最小磁盘预留空间 (MB)')}
                  extraText={t('始终保留这部分可用磁盘空间')}
                  min={0}
                  onChange={handleFieldChange(
                    'performance_setting.disk_cache_min_free_space_mb',
                  )}
                  disabled={
                    !inputs['performance_setting.disk_cache_enabled'] ||
                    !inputs['performance_setting.disk_cache_auto_sizing']
              ...
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.InputNumber
                  field={'performance_setting.disk_cache_max_request_mb'}
                  label={t('单请求最大缓存 (MB)')}
                  extraText={t('设为 0 表示不增加单请求限制')}
                  min={0}
                  onChange={handleFieldChange(
              ...
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.Switch
                  ...
                  label={t('未知长度请求优先写入磁盘')}
                  extraText={t('避免流式上传在内存中持续增长')}
                  onChange={handleFieldChange(
                    'performance_setting.disk_cache_unknown_length_disk_first',
                  )}
                  disabled={!inputs['performance_setting.disk_cache_enabled']}
