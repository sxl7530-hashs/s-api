...
export async function getAffiliateTransferLogs(
  page = 1,
  pageSize = 20
): Promise<ApiResponse<AffiliateTransferLogsResponse>> {
  const res = await api.get(
    `/api/user/self/aff_transfer_logs?p=${page}&page_size=${pageSize}`
  )
  return res.data
}
