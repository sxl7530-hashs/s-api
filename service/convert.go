...
// convertGeminiImageConfigToOpenRouter converts Gemini imageConfig to OpenRouter format:
// - Key names: camelCase → snake_case (aspectRatio → aspect_ratio)
// - Value formats: Gemini enum → OpenRouter value (PORTRAIT_9x16 → 9:16, IMAGE_SIZE_2K → 2k)
func convertGeminiImageConfigToOpenRouter(config map[string]interface{}) map[string]interface{} {
	...
	// Gemini aspect ratio enum → OpenRouter format
	geminiAspectRatioMap := map[string]string{
		"PORTRAIT_9x16":  "9:16",
		"LANDSCAPE_16x9": "16:9",
		"SQUARE_1x1":     "1:1",
		"PORTRAIT_3x4":   "3:4",
	...
	// Gemini image size enum → OpenRouter format
	geminiImageSizeMap := map[string]string{
		"IMAGE_SIZE_1K": "1K",
		"IMAGE_SIZE_2K": "2K",
		"IMAGE_SIZE_4K": "4K",
	}
	...
	for key, value := range config {
		...
		// Convert value format for known fields
		if strVal, ok := value.(string); ok {
			switch outKey {
			case "aspect_ratio":
				if converted, found := geminiAspectRatioMap[strVal]; found {
					value = converted
				}
				// else: already OpenRouter format (e.g. "9:16"), pass through
			case "image_size":
				if converted, found := geminiImageSizeMap[strVal]; found {
					value = converted
				}
				// else: already OpenRouter format (e.g. "2K"), pass through
			}
			_ = strVal
		}
...
func ResponseOpenAI2Gemini(openAIResponse *dto.OpenAITextResponse, info *relaycommon.RelayInfo) *dto.GeminiChatResponse {
	...
	for _, choice := range openAIResponse.Choices {
		...
		// 先处理消息内容：OpenRouter images 数组（图片生成响应）+ 多模态 content
...
func StreamResponseOpenAI2Gemini(openAIResponse *dto.ChatCompletionsStreamResponse, info *relaycommon.RelayInfo) *dto.GeminiChatResponse {
	...
	for _, choice := range openAIResponse.Choices {
		if len(choice.Delta.GetContentString()) > 0 || (choice.Delta.ToolCalls != nil && len(choice.Delta.ToolCalls) > 0) || len(choice.Delta.Images) > 0 {
			hasContent = true
		}
	...
	for _, choice := range openAIResponse.Choices {
		...
		if choice.Delta.ToolCalls != nil {
			...
			// 先尝试处理 OpenRouter 的 images 数组（图片生成流式响应）
			if len(choice.Delta.Images) > 0 {
				var images []dto.OpenRouterImage
				if err := common.Unmarshal(choice.Delta.Images, &images); err == nil {
					for _, img := range images {
						if img.Type == "image_url" && img.ImageUrl != nil {
							inlineData := parseBase64DataURL(img.ImageUrl.Url)
							if inlineData != nil {
