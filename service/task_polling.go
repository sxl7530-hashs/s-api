...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) {
	...
	// CUSTOM(Feature 4): Kling 平台用精确可灵点 + KlingPointRatio 重算
	if klingActualQuota := computeKlingActualQuota(ctx, task, taskResult); klingActualQuota > 0 {
		RecalculateTaskQuota(ctx, task, klingActualQuota, "Kling 可灵点结算")
		return
	}
...
// CUSTOM(Feature 4): Kling 平台精确计费
// 公式: 可灵点(精确小数) × KlingPointRatio × groupRatio × QuotaPerUnit
// 1 可灵点 = 1 元，KlingPointRatio 用于调整售价（如 0.78 表示 78% 折扣）
// 优先从 task.Data 中解析精确的 final_unit_deduction（可能为 "2.5" 这种小数），
// 否则回退用 taskResult.TotalTokens（已向上取整的整数值）。
// 返回 0 表示非 Kling 任务或没有可结算数据，调用方应继续走标准流程。
func computeKlingActualQuota(ctx context.Context, task *model.Task, taskResult *relaycommon.TaskInfo) int {
	if string(task.Platform) != strconv.Itoa(constant.ChannelTypeKling) {
		return 0
	}
	var klingPoints float64
	if len(task.Data) > 0 {
		var dataObj struct {
			Data struct {
		...
		if json.Unmarshal(task.Data, &dataObj) == nil && dataObj.Data.FinalUnitDeduction != "" {
			if parsed, err := strconv.ParseFloat(dataObj.Data.FinalUnitDeduction, 64); err == nil && parsed > 0 {
				klingPoints = parsed
			}
		}
	}
	if klingPoints <= 0 && taskResult.TotalTokens > 0 {
		klingPoints = float64(taskResult.TotalTokens)
	}
	if klingPoints <= 0 {
		return 0
	}
	...
	finalGroupRatio := 1.0
	if group != "" {
		if userGroupRatio, has := ratio_setting.GetGroupGroupRatio(group, group); has {
	...
	klingPointRatio := ratio_setting.GetKlingPointRatio()
	actualQuota := int(klingPoints * klingPointRatio * finalGroupRatio * common.QuotaPerUnit)
	logger.LogInfo(ctx, fmt.Sprintf("Kling 任务 %s 结算: 可灵点 %.4f × 倍率 %.4f × 分组倍率 %.4f = %d",
		task.TaskID, klingPoints, klingPointRatio, finalGroupRatio, actualQuota))
	return actualQuota
}
